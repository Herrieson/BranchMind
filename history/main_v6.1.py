...
class ContextTree:
    ...
    def plan_operation(self, decision: TreeDecision) -> PlannedOperation:
        ...
        if operation in (TreeOperationType.MERGE, TreeOperationType.SPLIT):
            ...
            secondary = [b for b in decision.secondary_branches if b in self.nodes]
...
"""
        ...
        try:
            raw_decision = self._call_llm(messages, model=self.config.cm_model, json_mode=True)
    ...
    def llm_task_execute(self, query: str, context: List[Dict[str, str]]) -> str:
        ...
        return self._call_llm(messages, model=self.config.task_model)
    ...
    def _summarize_interaction(self, query: str, response: str) -> str:
        ...
        summary = self._call_llm(messages, model=self.config.summarizer_model)
...
def main() -> None:
    ...
    print("\n欢迎来到 BranchMind V5 —— 支持跨分支推理与原子操作的改进版。")
    print("输入 'exit' 退出。")
    print("-" * 20)
    try:
        while True:
