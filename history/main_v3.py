...
class TreeDecision:
    ...
    def from_raw(cls, raw: Dict, root_id: str) -> "TreeDecision":
        ...
        primary = raw.get("primary_branch") or None
        if primary == "root":
            primary = root_id

        def _normalize_list(key: str) -> List[str]:
            values = raw.get(key) or []
            return [val if val != "root" else root_id for val in values]
...
class ContextTree:
    ...
    def plan_operation(self, decision: TreeDecision) -> PlannedOperation:
        ...
        primary = _ensure_exists(decision.primary_branch, "primary_branch")
        secondary = [ _ensure_exists(b, "secondary_branch") for b in decision.secondary_branches ]
        context_ids = [
            cid for cid in {
                *([primary] if primary else []),
                *secondary,
                *decision.context_branches,
            }
        ...
        if operation == TreeOperationType.NEW_BRANCH:
            ...
            parent_id = "root"
            archive_targets = list({*archive_targets, *secondary})
            ...
            parent_id = parent_node.parent_id or "root"
...
4. "split"     —— 如果某个分支主题过载，先记录分支，再从其父节点开启一个新的子分支。
...
def main() -> None:
    ...
    print("\n欢迎来到 BranchMind V3 —— 支持跨分支推理与原子树操作的版本。")
    print("输入 'exit' 退出。")
    print("-" * 20)
    while True:
